import type { DetectionResult, SecurityEvent, SecurityIncident, SecurityIncidentActivity, SecurityIncidentNote, SecurityIoc, SecurityIocType, SecuritySeverity, SecurityIocContext } from './types';
...
async function request<T>(path: string, init?: RequestInit): Promise<T> { const response = await fetch(`${API_BASE_URL}${path}`, { ...init, headers: { 'Content-Type': 'application/json', ...(init?.headers ?? {}) } }); const payload = (await response.json().catch(() => null)) as ApiEnvelope<T> | { error?: string } | null; if (!response.ok) throw new Error(payload?.error || `J.A.R.V.I.S. API request failed (${response.status})`); if (!payload || !('success' in payload) || !payload.success || !('data' in payload)) throw new Error('J.A.R.V.I.S. API returned an invalid response'); return payload.data; }
...
export const securityApi = {
  ...
  iocs: () => request<SecurityIoc[]>('/api/security/iocs'), iocContext: (id: string) => request<SecurityIocContext>(`/api/security/iocs/${encodeURIComponent(id)}/context`),
