...
const severityRank: Record<SecuritySeverity, number> = { low: 1, medium: 2, high: 3, critical: 4 };
...
export default function Incidents() {
  ...
  const incidents = useMemo<Incident[]>(() => {
    ...
    for (const detection of snapshot.detections) {
      const key = detection.title;
      const existing = grouped.get(key);
      const eventTime = detection.timestamp;
      const severity = existing && severityRank[existing.severity] > severityRank[detection.severity]
        ? existing.severity
        : detection.severity;
      ...
      if (existing) {
        ...
        existing.created = new Date(eventTime).toLocaleString();
        continue;
      }
      ...
      grouped.set(key, {
        ...
        severity,
        status: detection.severity === 'critical' ? 'investigating' : 'open',
        created: new Date(eventTime).toLocaleString(),
          ...
          <div className="max-w-7xl mx-auto px-6 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge className="bg-cyan-500/10 text-cyan-300 border-cyan-500/30 border gap-2"><Radio className="w-3 h-3" />LIVE</Badge>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          {incidents.length === 0 ? (
            <Card className="glass glow-border p-10 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Run a scenario in Attack Lab to create detection evidence.</p>
            </Card>
          ) : (
                ...
                <Card key={incident.id} className="glass glow-border p-6 hover:shadow-lg transition-all cursor-pointer">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4 border-t border-border/20">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <div><p className="text-xs text-muted-foreground">Last observed</p><p className="font-mono text-xs">{incident.created}</p></div>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <User className="w-4 h-4 text-muted-foreground" />
                      <div><p className="text-xs text-muted-foreground">Assignee</p><p className="font-mono text-xs">{incident.assignee}</p></div>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <FileText className="w-4 h-4 text-muted-foreground" />
                      <div><p className="text-xs text-muted-foreground">Evidence</p><p className="font-mono text-xs">{incident.evidence} events</p></div>
                    </div>
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="glass glow-border p-6">
              <p className="text-sm text-muted-foreground mb-2 font-mono">Detection Evidence</p>
              <p className="text-3xl font-mono font-bold text-cyan-400">{snapshot.detections.length}</p>
              <p className="text-xs text-muted-foreground mt-2">Security engine detections</p>
            </Card>
            <Card className="glass glow-border p-6">
              <p className="text-sm text-muted-foreground mb-2 font-mono">Critical Incidents</p>
              <p className="text-3xl font-mono font-bold text-red-400">{incidents.filter(i => i.severity === 'critical').length}</p>
