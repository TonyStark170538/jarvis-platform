...
function highestSeverity(values: SecuritySeverity[]): SecuritySeverity {
  return values.reduce<SecuritySeverity>(
...
async function createIncidentFromDetection(
  ...
  const existing = existingIncidents.find((incident) =>
    incident.status !== 'resolved' && incident.eventIds.some((id) => recentEventIds.includes(id))
  );
